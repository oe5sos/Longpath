...
| [2026-09-09-ci-discovery-hang-investigation.md](docs/architecture/2026-09-09-ci-discovery-hang-investigation.md) | 11 GUI tests hitting CTest's 120s `TIMEOUT` on Linux CI only (never macOS), all constructing a `MainWindow`. Two confirmed bugs in `RadioDiscovery`: every window open ran the full synchronous NIC-walk *twice* (a redundant direct `startDiscovery()` call in `MainWindow`'s ctor, immediately superseded by `ConnectionPanel`'s own ctor doing the identical scan again — reproduced locally on macOS with a single real NIC, ruling out a CI-only duplicate-interface theory for the doubling), and the per-attempt quiet-poll loop (`RadioDiscovery::scanAllNics`, now `quietPollAttempt()`) had no wall-clock bound at all — any sustained "readable" condition, whatever produces it, could hold its only counter at 0 forever | **Both fixed 2026-09-09: redundant scan removed (`MainWindow.cpp`, measured ~2.2s/window saved); quiet-poll loop bounded by a `QDeadlineTimer` safety net, proven by a new flood-based regression test (`tst_radio_discovery_scan_bound`). The exact Linux-only trigger that turned a bounded scan into a 120s+ hang was NOT identified — no Linux/Docker/act access this session — but the fix closes the hang possibility regardless, by bounding the one primitive every candidate cause would have to funnel through** |
