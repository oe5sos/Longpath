...
| [2026-08-29-sunsdr-tci-teardown-segfault-investigation.md](docs/architecture/2026-08-29-sunsdr-tci-teardown-segfault-investigation.md) | `tst_sunsdr_spectrum_wiring` (the older TCI-client SunSDR path, not the native driver above) — a real, symbolicated SIGSEGV crash report from a full parallel test run, crash site pinned exactly (`FFTRouter::removeReceiver()` via `MainWindow::wireSunSdr()`'s `TciClient::stateChanged` handler, inside Qt's own `QMap` copy-on-write detach), several theories ruled out with evidence, root cause still open | **Investigated, not fixed — has not reproduced in ~4 follow-up attempts; needs a live debugger session, not a guess, per this file's own "don't land a speculative fix" conclusion** |
