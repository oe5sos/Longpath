...
| [2026-09-17-cw-decoder.md](docs/architecture/2026-09-17-cw-decoder.md) | Native CW decoder ported from the Zeus station engine (GPL-2.0-or-later, `Station.Engine.Hosting/CwDecoder/` @8970f2d — Goertzel bank + adaptive threshold + dit estimator + Morse FSM; no Thetis/AetherSDR equivalent, sole source like the RTTY decoder). `CwDecoderCore` (pure C++, cited line by line), `CwDecoder` (Qt wrapper, no thread), `CwDecoderApplet` (visible only in CWL/CWU, same availability axis as RADE/RTTY), fifth `AudioEngine` tap. Two documented deviations in the timing estimator (gap thresholds 2.0/5.0 instead of 3.0/5.5; WPM from tone+gap clusters) found by the synthetic-Morse tests. First port recorded in `docs/attribution/ZEUS-PROVENANCE.md` | **Built 2026-09-17 (operator: "egal" on which benchmark item first), 9+3 tests green, applet rendered via the automation bridge; live CW check on the ANAN still pending** |
