...
| [2026-09-06-rtty-decoder-scoping.md](docs/architecture/2026-09-06-rtty-decoder-scoping.md) | Scoping-only research for a native RTTY (Baudot/ITA2) decoder, prompted by a general "what digital modes could Longpath do" question. Neither Thetis nor AetherSDR decode FT8/FT4/PSK31 themselves (both just route audio to WSJT-X/fldigi via VAC + CAT, which Longpath already has via DIGU/DIGL + `WsjtxClient`) — but AetherSDR has a genuine from-scratch RTTY decoder (`RttyDecoder.{h,cpp}`, mark/space biquad filters + Schmitt-trigger bit slicing + Baudot table decode) with no Thetis equivalent, sole-source like the automation bridge. Realistic total port size ~700-1000 lines including a new Longpath-style applet UI (AetherSDR has no `PanadapterApplet` equivalent in Longpath) | **Research only, no code — awaiting a build/no-build decision** |
