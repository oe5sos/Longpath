...
* `RadioConnection` — Protocol 1 (UDP) and Protocol 2 (UDP multi-port) connections. A third subclass, `SunSdrRadioConnection` (`src/core/SunSdrRadioConnection.h`), talks the SunSDR2 QRP's own native wire protocol directly (no ExpertSDR2 middleman, no OpenHPSDR framing at all) — see [2026-08-24-sunsdr-native-driver-design.md](docs/architecture/2026-08-24-sunsdr-native-driver-design.md).
...
* `MemoryRecord` / `MemoryList` (`src/models/`) + `MemoryDialog` (`src/gui/`) — Thetis frequency memories: one record per slot, the list as a QAbstractTableModel over `memory.xml` (Thetis's own format, interchangeable), the Memory window; `RadioModel::captureMemory` / `recallMemory` / quick memory. Added 2026-09-20.
* `SettingsBackup` (`src/core/`) + `SettingsBackupDialog` (`src/gui/`) — the backup half of Thetis's Database Manager: copies of the settings file in `backups/` with a `{Description, Auto}` JSON sidecar, restore (safety copy, then close without saving via `AppSettings::setSaveInhibited`), start-up/shut-down copies, GFS prune of automatic copies. File > Settings Backups... Added 2026-09-20.
* `RxProfileManager` (`src/core/`) + `RxProfilePopup` (`src/gui/widgets/`) — receive profiles: 71 SliceModel Q_PROPERTYs (AGC / NR / ANF / NB / squelch / APF / binaural) captured via QObject::property and applied via QMetaProperty::write under a name (`RxProfile/<name>/<property>` in AppSettings, global); the sheet behind the RX applet's gear. Added 2026-09-20.
* `CwDecoder` (`src/core/`) + `CwDecoderApplet` (`src/gui/applets/`) — native CW decoder: ggmorse (MIT, vendored byte-identical at `third_party/ggmorse/`) behind a worker-thread wrapper ported from AetherSDR; fed from `AudioEngine::setCwTap` (48 kHz), visible in CWL/CWU on the applet availability axis like `RttyDecoderApplet`. Added 2026-09-21.
...
| [2026-08-26-sunsdr-native-verification/README.md](docs/architecture/2026-08-26-sunsdr-native-verification/README.md) | SunSDR2 QRP (native driver, no ExpertSDR2): 10-row bench verification matrix | Matrix verified rows 1-4 (connect/panadapter/audio/frequency, 2026-08-26/27, OE5SOS); rows 5-10 (client-side mode/filter, reconnect cycle, dead-radio detection, ExpertSDR2-coexistence, TX-inertness, dual-radio coexistence) pending — row 7 (dead-radio detection) has a client-side watchdog now, added 2026-08-28, not yet bench-confirmed |
| [2026-08-29-sunsdr-tci-teardown-segfault-investigation.md](docs/architecture/2026-08-29-sunsdr-tci-teardown-segfault-investigation.md) | `tst_sunsdr_spectrum_wiring` (the older TCI-client SunSDR path, not the native driver above) — a real, symbolicated SIGSEGV crash report from a full parallel test run, root cause confirmed via AddressSanitizer (heap-use-after-free: `MainWindow::wireSunSdr()`'s `TciClient::stateChanged` handler touches `m_radioModel` after it's freed, during `MainWindow`'s own reentrant teardown), plus the fix that closes an AddressSanitizer gap in the test build itself (`tests/CMakeLists.txt` never linked the ASAN runtime for test executables) | **RESOLVED 2026-08-30 — one-line `m_shuttingDown` guard, same pattern this class already uses elsewhere; ASAN caught the bug deterministically before the fix and is clean 3/3 after; 7/7 directly relevant tests green on the normal build** |
| [2026-09-06-dev-automation-bridge-design.md](docs/architecture/2026-09-06-dev-automation-bridge-design.md) | Structural derivative of AetherSDR's agent automation bridge (`AutomationServer`, issue #3646): an opt-in (`LONGPATH_AUTOMATION`), in-process `QLocalServer` exposing widget-tree JSON (`dumpTree`) and per-widget PNG capture (`grab`, including the GPU panadapter via `QRhiWidget::grabFramebuffer()`) — lets an agent verify a GUI change without computer-use screenshots. Phased plan; AetherSDR's own version is ~12,000 lines across 25 verbs | **Phase 0 (read-only: `ping`/`dumpTree`/`grab`/`get radio`/`get slice`) shipped 2026-09-06, dogfooded live against the real app including a full-resolution panadapter capture. Phase 1+ (interact verbs, TX-safety gating) scoped but not started** |
| [2026-09-06-midi2cat-design.md](docs/architecture/2026-09-06-midi2cat-design.md) | Scoping-only research for a Thetis Midi2Cat port (MIDI controller → CAT command mapping, e.g. Behringer BCF2000/BCR2000): corrects an earlier "~6700 line" estimate to ~13,300 (Thetis's `Console/Midi2CatCommands.cs` was missed entirely), full 236-command inventory with TX-relevant commands (MOX/TUNE/CWX/two-tone/QuickPlay) flagged for the same separate-opt-in TX-safety gate the automation bridge uses, notes AetherSDR already vendors RtMidi with a mature `MidiControlManager`/`MidiSettings`/`MidiRelativeCcDecoder`/`MidiMappingDialog` stack to cite structurally | **Research only, no code. Operator has no MIDI controller — shelved, not being pursued** |
| [2026-09-06-rtty-decoder-scoping.md](docs/architecture/2026-09-06-rtty-decoder-scoping.md) | Native RTTY (Baudot/ITA2) decoder, prompted by a general "what digital modes could Longpath do" question. Neither Thetis nor AetherSDR decode FT8/FT4/PSK31 themselves (both just route audio to WSJT-X/fldigi via VAC + CAT, which Longpath already has via DIGU/DIGL + `WsjtxClient`) — but AetherSDR has a genuine from-scratch RTTY decoder (`RttyDecoder.{h,cpp}`, mark/space biquad filters + Schmitt-trigger bit slicing + Baudot table decode) with no Thetis equivalent, sole-source like the automation bridge. New `RttyDecoderApplet` (visible only in DSPMode::DIGL, matching `RxApplet::applyModeVisibility`'s existing rule), reads mark/shift live from the already-existing, Thetis-sourced `SliceModel::rttyMarkHz`/`rttyShiftHz` instead of owning a second copy | **Shipped 2026-09-06 (operator: "ja bitte rtty"), live-verified 2026-09-07 against a real ANAN 10e: docked panel appeared correctly in DIGL, read live Mark/Shift from the slice, decoded (garbage, no antenna, as expected). Decode logic separately verified end-to-end against a synthetic AFSK signal (`tests/tst_rtty_decoder.cpp`)** |
| [2026-09-09-ci-discovery-hang-investigation.md](docs/architecture/2026-09-09-ci-discovery-hang-investigation.md) | 26 GUI tests (initially reported as 11) hitting CTest's 120s `TIMEOUT` on Linux CI only (never macOS), all constructing a `MainWindow`. First pass fixed two real, confirmed bugs in `RadioDiscovery` (a redundant double NIC-walk on every window open; the per-attempt quiet-poll loop having no wall-clock bound) — merged, then proven by the *next* CI run to be a red herring: byte-for-byte identical failures before and after. Actual cause found on the second pass: `MainWindow::showAudioDiagnoseDialog()` (Linux-only) calls modal `dlg->exec()` with no timeout, unconditionally scheduled whenever `linuxBackend() == None` (every Linux CI runner — no ALSA/JACK hardware, confirmed verbatim in every failing test's own log) and `Audio/LinuxFirstRunSeen` is unset (every test's fresh `QStandardPaths` sandbox) — both always true in CI, so the dialog pops and blocks forever with nothing to dismiss it | **RadioDiscovery fix shipped 2026-09-09 (PR #9, `988fa77e`) — real bugs, kept, but did not move the failures at all. Actual fix (same day, separate PR): `showAudioDiagnoseDialog()` returns immediately when `QStandardPaths::isTestModeEnabled()`, a no-op for real users (only test binaries ever set that flag) and a guaranteed skip for every automated run. Not yet confirmed on live Linux CI (no Linux/Docker/act access this session) but the mechanism has no environmental non-determinism left in it — both gating conditions are logged verbatim in every failing test** |
| [2026-09-20-memories.md](docs/architecture/2026-09-20-memories.md) | Frequency memories: Thetis `Memory/MemoryRecord.cs` + `MemoryList.cs` + `MemoryForm.cs` and `console.cs` RecallMemory / quick memory ported [@852bf0e]. `MemoryRecord` (20 fields, upstream defaults, tune-step name table), `MemoryList` (QAbstractTableModel + `memory.xml` in Thetis's own XmlSerializer layout with `memory_bak.xml` fallback, unknown elements round-tripped), `MemoryDialog` (grid + Add/Copy/Delete/Select + close-after-select), `RadioModel::captureMemory/recallMemory/memoryQuickSave/memoryQuickRestore`, Tools menu entries. Deviations: no Filter enum (bounds applied), split/TXFreq + FM deviation stored not applied, ke9ns schedule fields carried as extras | **Built 2026-09-20, 15 test cases green, dialog rendered from the test; live check on a radio pending** |
| [2026-09-20-settings-backup.md](docs/architecture/2026-09-20-settings-backup.md) | Settings backups: the backup half of Thetis `clsDBMan.cs` + `frmDBMan.cs` ported [@852bf0e]. `SettingsBackup` (copy + JSON sidecar, newest-first listing, rename/remove/export/restore, GFS prune of automatic copies only), `SettingsBackupDialog` (list, Take Backup Now / Restore / Rename / Export / Remove / Open Folder, the three switches), start-up copy in `main.cpp`, shut-down copy after the final save, File menu entry. Deviations: one file instead of GUID databases, restore = copy over the live file + close (no self-restart), flush before a manual copy, epoch from the file name | **Built 2026-09-20, 24 test cases green, start-up/shut-down/prune verified live in a sandbox profile; menu click + restore round trip in the running app pending** |
| [2026-09-20-rx-profiles.md](docs/architecture/2026-09-20-rx-profiles.md) | Receive profiles (Longpath-original, no port): which SliceModel properties a profile carries and which deliberately not (frequency memories' business), the property-name-driven capture/apply, the AppSettings layout, the popup behind the RX applet's gear (inline name field + inline delete confirmation because Qt::Popup closes under modal boxes), the four decisions taken without the operator | **Built 2026-09-20, 11 test cases green, sheet rendered from the test; gear click in the running app pending** |
| [2026-09-21-cw-decoder.md](docs/architecture/2026-09-21-cw-decoder.md) | Native CW decoder: ggmorse (MIT, pristine upstream @8fb433d6, `GGMORSE-PROVENANCE.md`) behind AetherSDR's wrapper ported [@e944ec49] (worker thread, parameter snapshot, ring, lock semantics), a fifth AudioEngine tap, `CwDecoderApplet` gated on CWL/CWU. Deviations: 48 kHz float in / mono float to ggmorse, no TX-side known-parameter path, ggmorse's own 5-55 WPM range (AetherSDR's speed-range patch not vendored), pitch band from the CW pitch setting +/- 150 Hz | **Built 2026-09-21; a synthetic 700 Hz / 20 WPM "CQ TEST DE OE5SOS" decodes word for word in the test; 9 test cases green; live on a CW signal pending** |

| [2026-09-18-logbook-stats.md](docs/architecture/2026-09-18-logbook-stats.md) | Logbook Kennzahlen + Awards (benchmark item 4 of the 2026-09-17 comparison; the reference client is the benchmark, nothing ported — it is closed source). `LogbookStats` (pure: totals, per band/mode/year, 26 weekly buckets, countries via cty.dat primary prefix with the six non-DXCC `*` rows folded into their DXCC entity, DXCC worked/confirmed overall + per band, ARRL WAS with DC→MD and implicit AK/HI, grids, continents, CQ zones), `LogbookStatsWidget` (six tiles, QPainter bar charts, no new dependency), replaces the text dump behind the logbook's "Stats…" as a modeless dialog over the filtered view | **Built 2026-09-18, 15 test cases incl. 400 synthetic QSOs against the real cty.dat, tiles rendered (PNG in the doc)** |
