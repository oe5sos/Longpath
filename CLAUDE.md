...
| [2026-09-06-rtty-decoder-scoping.md](docs/architecture/2026-09-06-rtty-decoder-scoping.md) | Native RTTY (Baudot/ITA2) decoder, prompted by a general "what digital modes could Longpath do" question. Neither Thetis nor AetherSDR decode FT8/FT4/PSK31 themselves (both just route audio to WSJT-X/fldigi via VAC + CAT, which Longpath already has via DIGU/DIGL + `WsjtxClient`) — but AetherSDR has a genuine from-scratch RTTY decoder (`RttyDecoder.{h,cpp}`, mark/space biquad filters + Schmitt-trigger bit slicing + Baudot table decode) with no Thetis equivalent, sole-source like the automation bridge. New `RttyDecoderApplet` (visible only in DSPMode::DIGL, matching `RxApplet::applyModeVisibility`'s existing rule), reads mark/shift live from the already-existing, Thetis-sourced `SliceModel::rttyMarkHz`/`rttyShiftHz` instead of owning a second copy | **Shipped 2026-09-06 (operator: "ja bitte rtty"). Decode logic verified end-to-end against a synthetic AFSK signal (`tests/tst_rtty_decoder.cpp`); on-screen visual check still pending (no interactive screen-control session available this run)** |
