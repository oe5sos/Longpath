...
| [2026-09-18-rx-leveler.md](docs/architecture/2026-09-18-rx-leveler.md) | RX audio leveler ported from the Zeus station engine (GPL-2.0-or-later, `DspPipelineService.cs` `ApplyRxAudioLevelerCore` + evidence gate, `InPassbandSnrEstimator.cs`, @8970f2d; "this always-on leveler (which Thetis lacks)"). Post-WDSP per-slice loudness correction: cut unconditional (in-block peak guard + soft limiter), boost fail-closed behind a spectrum-derived in-passband SNR verdict and an ADC-overload veto. `RxAudioLeveler` (pure C++), `InPassbandSnrEstimator` (pure C++), `PassbandSnrTracker` (Longpath glue: FFTEngine linear frames → WDSP-avenger averaging → 5 Hz verdicts → RxChannel atomics), `LVL` toggle + right-click profile in the RxApplet. Four documented deviations (D1 stereo pair, D2 block-rate scaling, D3 same-spectrum cross-check, D4 off = bypass with a release fade) | **Built 2026-09-18, 42 test cases green incl. an end-to-end tracker→WDSP-channel test, applet rendered via the automation bridge; live check on a radio pending (all radios were held by other instances at build time)** |
