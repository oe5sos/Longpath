...
| `src/main.cpp` | Port `clsDBMan.cs:370-380` | **Startup-Kopie** direkt nach `load()`, vor den Schema-Migrationen — also von der Datei, wie die letzte Sitzung sie hinterließ (Thetis kopiert vor `DB.Init()`). |
| `MainWindow` | Port `console.designer.cs:4125-4128`, `clsDBMan.cs:541-557`, `:975-978` | **File → Settings Backups…** (Thetis: Setup → Database Manager); **Shutdown-Kopie** nach dem letzten `AppSettings::save()` auf beiden Beenden-Wegen (closeEvent und aboutToQuit, einmalig); nach einer Wiederherstellung: Dialog weg, Sperre setzen, Beenden über den Cmd+Q-Weg. |
