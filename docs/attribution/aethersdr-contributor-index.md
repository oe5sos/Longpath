...
| `src/gui/VfoWidget.{h,cpp}` | `src/gui/widgets/VfoWidget.{h,cpp}` / `VfoModeContainers.{h,cpp}` / `VfoLevelBar.{h,cpp}` / `SliceColors.h` | **ZURUECKGEZOGEN 2026-08-18 — VfoWidget, VfoModeContainers und VfoLevelBar sind geloescht.** Die schwebende VFO-Flagge faellt ersatzlos weg; letzter Stand in Commit `300e8d48`. Was UEBERLEBT hat, mit eigener Attribution: `SliceColors.h` (die vier Scheibenfarben aus AetherSDR `SliceColors.h` [@0cd4559], herausgeloest weil RxApplet sie teilt) und `gui/widgets/DspQuickPopups.{h,cpp}` (NR-Schnellregler, eigene Thetis-PROVENANCE-Zeile). `VfoStyles.h` und `ScrollableLabel.{h,cpp}` bleiben — sie bedienen HGauge, VaxChannelSelector und DexpPeakMeter. |
...
| ↳ | **am 2026-08-20 aus dem Baum entfernt** | Ersetzt durch `BandwidthFilterPane` (Longpath-original, Frequenzachse statt festem Trapez). Die Zeile bleibt stehen: der Port HAT stattgefunden, und der Code lebt in der Geschichte weiter. Letzter Commit mit der Datei: `1b46792c`. |
