...
| `src/core/audio/RxAudioLeveler.{h,cpp}` | `Station.Engine.Hosting/DspPipelineService.cs`, `Zeus.Contracts/Dtos.cs`, `Station.Engine.Hosting/RadioService.cs` | DspPipelineService.cs 71, 174-194, 213-280, 336-363, 384-701, 839-848, 4453-4456, 10245-10364; Dtos.cs 862-893; RadioService.cs 4248-4266 | port | The RX audio leveler ("RX LVLR"): state, constants, `ApplyRxAudioLeveler` + `ApplyRxAudioLevelerCore`, `SoftLimitRxAudioSample`, `AudioLinearToDbfsRaw`, the constant-loudness evidence gate (`AdvanceRxConstantLoudnessEvidence`, `RxConstantLoudnessEvidenceIsCurrent`, `RxConstantLoudnessEvidenceAllowsBoost`, the ADC-risk rule) and the `RxLevelerConfig` profile with its normalisation. Ported 2026-09-18 @8970f2d (v2.0.26). Four documented deviations, marked at the site: D1 one gain over both WDSP legs; D2 per-block constants scaled by block duration / (1000/30 ms); D3 the cross-check's "current signal" is the current-frame in-passband power from the same spectrum, not the WDSP stage meter; D4 off = bypass after a release fade (upstream keeps cut + limiter running while off). Tests `tests/tst_rx_audio_leveler.cpp`. |
| `src/core/InPassbandSnrEstimator.{h,cpp}` | `Station.Engine.Hosting/InPassbandSnrEstimator.cs` | 1-290 (whole file) | port | In-passband SNR from a dB/Hz spectrum: guarded reference regions, robust median/MAD sides, slope-interpolated noise integral, 3.5-sigma detection, confidence; per-key settle + keyed restart. Ported 2026-09-18 @8970f2d, arithmetic unchanged (LINQ → std::sort/loops). Tests `tests/tst_in_passband_snr_estimator.cpp`. |
| `src/models/PassbandSnrTracker.{h,cpp}` | `Zeus.Dsp/Wdsp/WdspDspEngine.cs` (`ConfigureSnrAnalyzer`, `ConfigureSnrPowerOutput`), `Station.Engine.Hosting/DspPipelineService.cs` (`UpdateRxConstantLoudnessEvidence`, the Tick call site) | WdspDspEngine.cs 4517-4534, 4553-4585; DspPipelineService.cs 9660-9690, 10052-10087, 10245-10287 | reference | Longpath-original glue that stands in for Zeus's second WDSP analyzer per receiver: same detector/averaging/normalisation choices (average-power, linear recursive tau 0.75 s, one-hertz) reproduced with WDSP's own `avenger()` arithmetic on FFTEngine's linear frames, same 5 Hz evidence cadence, same evidence bookkeeping order. No Zeus code copied beyond the evidence-update sequence, which is cited. Tests `tests/tst_passband_snr_tracker.cpp`. |
