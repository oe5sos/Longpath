...
| `src/core/CwDecoder.h`, `.cpp` | `src/core/CwDecoder.{h,cpp}` | `e944ec49` | 2026-09-21 | Structural and behavioral derivative of AetherSDR's ggmorse wrapper (the state after `6e58e633`, "Fix CW decoder parameter races"): worker thread, one coherent pending-parameter snapshot, mono ring drained one ggmorse frame per call, lock semantics, queued-at-delivery statistics. Translated for Longpath's audio-tap convention (48 kHz interleaved float32 in, mono float32 to ggmorse -- AetherSDR downmixes 24 kHz stereo to int16). `setKnownParameters` (TX-side decoder) and the speed range (an AetherSDR patch to ggmorse, not vendored here) are not carried over. ggmorse itself is vendored pristine (MIT): `GGMORSE-PROVENANCE.md`. |
| `src/gui/applets/CwDecoderApplet.h`, `.cpp` | `src/gui/PanadapterApplet.cpp` (CW decode control set) | `e944ec49` | 2026-09-21 | Structural derivative, not a port: the control set (decoded text, pitch/speed readout, lock-pitch / lock-speed toggles, Clear) against Longpath's own `AppletWidget`, the same shape as `RttyDecoderApplet`. Pitch band = the operator's CW pitch (AppSettings `CWPitch`, Thetis-sourced) +/- 150 Hz, after AetherSDR's `setKnownParameters` padding. Mode gate `DSPMode::CWL`/`CWU` in MainWindow, the RTTY applet's pattern. |
...
| `src/core/KiwiSdrManager.h`, `.cpp` | `src/core/KiwiSdrManager.{h,cpp}` | `31b29583` | 2026-08-23 | Namespace change only. 2026-09-21 (Longpath): signal `sliceTrackingUpdated` emitted from `assignSliceToProfile` / `updateSliceTracking` -- the observable edge for the tracking test; no upstream equivalent. |
| `src/core/KiwiSdrProtocol.h`, `.cpp` | `src/core/KiwiSdrProtocol.{h,cpp}` | `31b29583` | 2026-08-23 | Namespace change only. 2026-09-21 (Longpath): `carrierSymmetricCwPassband` -- Longpath's CW filters are pitch-centred (Thetis), upstream's are carrier-symmetric (Flex); takes the pitch back out before `formatSoundTuneCommand` adds it. No upstream equivalent. |
...
| `src/gui/MainWindow_KiwiSdr.cpp` | `src/gui/MainWindow_KiwiSdr.cpp` | `31b29583` | 2026-08-23 | Stufe 4 (Bedienflaeche). Upstream is ~2429 lines across ~30 `MainWindow` methods; most depend on features Longpath doesn't have yet (per-slice virtual antennas, band recall, diversity — Stufe 7, deliberately deferred). This file ports the bridge that does exist: manager state → applet display, audio + waterfall wiring. 2026-09-21: tracking follow (Stufe 3b) -- `rewireKiwiSdrTrackingForSlice` / `updateKiwiSdrTrackingForSlice` carry over what upstream's `updateKiwiSdrVirtualTrackingForSlice` and its per-slice connects in `MainWindow_Wiring.cpp` do, wired per assignment instead of per slice, without virtual antennas / band recall. |
...
| `src/core/dsp/FftwPlannerLock.h`, `.cpp` | `src/core/dsp/FftwPlannerLock.{h,cpp}` | `7f68dda0` | 2026-09-22 | Namensraum; die Aufzaehlung der Aufrufstellen im Kopf ist Longpaths eigene (FFTEngine, WidebandFftEngine, RxChannel, WDSP-Kanaele und der WDSPwisdom-Faden). Die Sperren selbst sind Wort fuer Wort dieselbe Loesung: je Genauigkeit ein prozessweiter Mutex, gehalten ueber Belegung, Plan, Verwerfen und Wissenszugriff, nie ueber fftw_execute. |
| `src/gui/ScopedChildWidget.h` | `src/gui/ScopedChildWidget.h` | `b9e96a94` | 2026-09-22 | Namensraum; die Begruendung im Kopf ist ausgeschrieben und nennt Longpaths eigenen Absturz vom 2026-08-30. Die Klasse selbst ist unveraendert: QPointer auf ein Kind auf dem Haufen, Freigabe im Destruktor nur wenn es noch da ist. |
